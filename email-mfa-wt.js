    ...
    jwt.verify(token, new Buffer(req.webtaskContext.secrets.token_secret, 'base64'), function(err, decoded) {
        ...
        if (decoded.email_identity === null) {
            getUserById(req.webtaskContext, decoded.sub, function(err, user) {
                if (err) {
                    res.end('Error while looking up user. Error - ' + JSON.stringify(err));
                    return;
                }
                console.log(user);
                ...
                if (email_connections.length === 0) {
                    createEmailPasswordlessAccount(req.webtaskContext, user.email || user.EMAIL, function(err, user) {
                        if (err) {
                            res.end('Error while creating verification user. Error - ' + JSON.stringify(err));
                            return;
                        }

                        linkAccounts(req.webtaskContext, decoded.sub, user.user_id, function(err, identities) {
                            if (err) {
                                res.end('Error while joining verification user to original. Error - ' + JSON.stringify(err));
                                return;
                            }
                            var email_connections = identities.filter(function(id) {
                                return id.provider === 'email' && id.profileData && id.profileData.email_verified;
                            });
                            var newTokenPayload = {};
...
function passwordlessSecondStepForm() {
    ...
    <html lang="en">
      <head>
        ...
        <meta charset="UTF-8">
        <title>Auth0 - email multifactor authentication</title>
      ...
      <body>
        <div class="modal-wrapper">
          <div class="modal-centrix">
            <div class="modal">
              <form onsubmit="showSpinner();" action="/email-mfa" method="POST" enctype="application/x-www-form-urlencoded">
                ...
                <div class="body"><span class="description">An email with the code has been sent to <%- email %>.</span>
                ...
                <div id="ok-button" class="ok-cancel">
                  <button class="ok full-width">
                    <span class="icon"><svg width="43px" height="42px" viewBox="0 0 43 42" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns"><g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" sketch:type="MSPage"><g id="Lock" sketch:type="MSArtboardGroup" transform="translate(-280.000000, -3592.000000)"><g id="email" sketch:type="MSLayerGroup" transform="translate(153.000000, 3207.000000)"><g id="Group" sketch:type="MSShapeGroup"><g id="Login" transform="translate(0.000000, 369.000000)"><g id="Btn"><g id="Oval-302-+-Shape" transform="translate(128.000000, 17.000000)"><circle id="Oval-302" stroke="#FFFFFF" stroke-width="2" cx="20.5" cy="20" r="20"></circle><path d="M17.8,15.4 L19.2,14 L25.2,20 L19.2,26 L17.8,24.6 L22.4,20 L17.8,15.4 Z" id="Shape" fill="#FFFFFF"></path></g></g></g></g></g></g></g></svg></span>
...
var createEmailPasswordlessAccount = (webtaskContext, email, cb) => {
    tools.managementApi.getClient({
            domain: webtaskContext.secrets.auth0_domain,
            clientId: webtaskContext.secrets.management_api_client_id,
            clientSecret: webtaskContext.secrets.management_api_client_secret
...
var linkAccounts = (webtaskContext, orig_user_id, email_user_id, cb) => {
    tools.managementApi.getClient({
            domain: webtaskContext.secrets.auth0_domain,
            clientId: webtaskContext.secrets.management_api_client_id,
            clientSecret: webtaskContext.secrets.management_api_client_secret
        ...
        .then(function(client) {
            ...
            client.users.link(orig_user_id, {
                provider: 'email',
